...
function init() {
    ...
    aniSeq=new sequence(animationV,timeTable);
...
function sequence(animation_seq,time_table){
    this.index=0;
...
function move1(position,finalPos,time){
    ...
    this.x=position[0];
    this.y=position[1];
    this.disx=finalPos[0]-x;
    this.disy=finalPos[1]-y;
    var xIterations=this.disx/time,yIterations=this.disy/time;
    
    
    this.position=position;
    this.nextPos=function(actual_time){
        if((actual_time%xIterations)==0){
            this.x+=xIterations;
        }
        if((actual_time%yIterations)==0){
            this.y+=yIterations;
        }
